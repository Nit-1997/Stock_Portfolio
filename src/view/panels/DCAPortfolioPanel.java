...
/**
 * Panel for DCA Strategy.
 */
public class DCAPortfolioPanel {
  ...
  JPanel printDCACreationMenu(Map<String, Double> dcaStockMap, SinglePortfolioDetailPanel main,
    ...
    if (portNameInput == null) {
      title.setText("Investment through Dollar Cost Averaging");
    }
    ...
    if (portNameInput == null) {
      ...
      portNameInput.setText(main.name);
      portNameInput.setEditable(false);
    } else {
    ...
    this.percentageInput.setColumns(10);
  ...
  private void printMoreLines(Map<String, Double> dcaStockMap,
    ...
    if (!this.inputValidation(dcaStockMap, confirmationMsg)) {
      return;
    }
  ...
  boolean inputValidation(Map<String, Double> dcaStockMap, JLabel confirmationMsg) {
    ...
    if (!Constants.STOCK_NAMES.contains(stock.toUpperCase())) {
      this.printForDCAPortfolioCreation(confirmationMsg, "Not a valid stock name",
          false);
      return false;
    }
    Double percent;
    try {
      ...
      this.printForDCAPortfolioCreation(confirmationMsg, "Number not in numeric format",
          false);
      return false;
    }
    if (percent == 0) {
    ...
    if (percent < 0 || percent > 100) {
      this.printForDCAPortfolioCreation(confirmationMsg, "Percentage -ve or 100+",
          false);
      return false;
    }
    if (this.sum + percent > 100.0) {
      this.printForDCAPortfolioCreation(confirmationMsg, "Weights sum more than 100",
          false);
      return false;
    }
    this.sum += percent;
