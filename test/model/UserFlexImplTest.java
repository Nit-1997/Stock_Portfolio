...
public class UserFlexImplTest {
  ...
  public void testConstructor(){
    ...
    assertEquals(file.list((f1, name) -> name.endsWith(".csv")).length,user.getPortfolios().size());
  ...
  public void testGetPortfolioSummaryBeforeCreationDate(){
    Map<String, Double> summary = user.getPortfolioSummary("flexUserTest2","2014-01-01");
    assertNull(summary);
  }
  ...
  public void testGetPortfolioSummaryAfterToday(){
    Map<String, Double> summary = user.getPortfolioSummary("flexUserTest2",LocalDate.now().plusDays(3).toString());
    assertNull(summary);
  }

  @Test
  public void testGetCurrentPortfolioSummary(){
    Map<String, Double> summary = user.getPortfolioSummary("flexUserTest2",LocalDate.now().toString());
    assertEquals(summary.size(),5);
    assertTrue(summary.containsKey("AAPL"));
    assertTrue(summary.containsKey("INTU"));
  ...
  public void testGetCostBasisAfterToday(){
    Double cost = user.getCostBasis("flexUserTest2", LocalDate.now().plusDays(10).toString());
    assertNull(cost);
  }
  ...
  public void testTransactionForPortfolioBuyNewStockAfterCurrentDate() throws Exception {
    boolean val = user.transactionForPortfolio("flexUserTest",
        new SimpleEntry<>("GOOG",new SimpleEntry<>(LocalDate.now().plusDays(1).toString(),new SimpleEntry<>(15.0,3.8))));
  }

  @Test(expected = Exception.class)
  public void testTransactionForPortfolioBuyNewStockWrongTicker() throws Exception {
    boolean val = user.transactionForPortfolio("flexUserTest",
        new SimpleEntry<>("fds",new SimpleEntry<>(LocalDate.now().toString(),new SimpleEntry<>(15.0,3.8))));
  }
  ...
  public void testGetGraphDataWeeklyInvalidRange(){
    ...
    graphdata = user.getGraphData("2016-08-13","2022-12-10","flexUserTest2");
    assertNull(graphdata);

    graphdata = user.getGraphData("2019-08-13","2018-12-10","flexUserTest2");
    assertNull(graphdata);
  }
