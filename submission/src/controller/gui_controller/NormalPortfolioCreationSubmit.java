...
/**
 * Normal portfolio creation class.
 */
public class NormalPortfolioCreationSubmit implements GUISubController {

  List<String> data;
  UserFlexInvest user;

  /**
   * Constructor for normal portfolio creation.
   *
   * @param data list of form data.
   * @param user model objet.
   */
  ...
  public String execute() {
    ...
    } else if (!user.isUniqueName(portfolioName)) {
      return "Please enter a unique portfolio Name";
    }

    // if nothing in form but in textArea
    if (data.get(1).equals("") && data.get(2).equals("") && data.get(3).equals("") && data.get(4)
        .equals("") && !data.get(5).equals("")) {
      try {
    ...
    if (data.get(5).equals("")) {
      if (!FormChecker.formChecker(data, user).equals("Portfolio Successfully Saved")) {
        return FormChecker.formChecker(data, user);
      }
      if (!user.isUniqueName(portfolioName)) {
        return "Please enter a unique portfolio Name";
      }
      ...
      dateMap.put(data.get(3),
          new SimpleEntry<>(Double.parseDouble(data.get(2)), Double.parseDouble(data.get(4))));
    ...
    if (stockMap2.containsKey(stock)) {
      if (stockMap2.get(stock).containsKey(date)) {
        stockMap2.get(stock).put(date,
            new SimpleEntry<>(stockMap2.get(stock).get(date).getKey() + quantity, commFee));
      } else {
        stockMap2.get(stock).put(date, new SimpleEntry<>(quantity, commFee));
      }
    } else {
  ...
  Map<String, Map<String, SimpleEntry<Double, Double>>> parseExtra() {
